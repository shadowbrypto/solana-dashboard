...
interface EVMMonthlyMetricsTableProps {
  protocols: Protocol[];
  endDate: Date;
...
const fetchEVMMonthlyData = async (protocols: Protocol[], endDate: Date): Promise<EVMProtocolData[]> => {
  ...
  try {
    ...
    console.error('Error fetching EVM monthly data:', error);
    
    // Return empty data if API fails
    return protocols.filter(p => p !== 'all').map(protocol => ({
      protocol,
      totalVolume: 0,
      chainVolumes: {
        ethereum: 0,
...
const SixMonthTrendChart: React.FC<{ data: number[]; growth: number }> = ({ data, growth }) => {
  const chartData = data.map((value, index) => ({ 
    month: index, 
    value 
  }));

  // Calculate trend direction based on 6-month data
  ...
  return (
    <div className="flex items-center justify-between w-full">
      <div className="w-[60px] h-[28px] -my-1">
...
export function EVMMonthlyMetricsTable({ protocols, endDate, onDateChange }: EVMMonthlyMetricsTableProps) {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        const data = await fetchEVMMonthlyData(protocols, endDate);
        ...
        console.error('Error loading EVM monthly data:', err);
  ...
  const totals = useMemo(() => {
    ...
    const totalPreviousVolume = visibleData.reduce((sum, data) => sum + data.previousMonthVolume, 0);
    ...
    // Calculate overall growth (total this month vs total last month)
    const overallGrowth = totalPreviousVolume > 0 
      ? (totalVolume - totalPreviousVolume) / totalPreviousVolume 
      : 0;

    // Calculate total 6-month trend (sum across all visible protocols for each month)
    const totalSixMonthTrend = visibleData.length > 0 && visibleData[0].sixMonthTrend.length > 0
      ? visibleData[0].sixMonthTrend.map((_, monthIndex) => 
          visibleData.reduce((sum, data) => sum + (data.sixMonthTrend[monthIndex] || 0), 0)
        )
      : [];
  ...
  const handleDateChange = (newDate?: Date) => {
    if (newDate) {
      onDateChange(endOfMonth(newDate));
    }
  };
  ...
  const downloadReport = async () => {
    const tableElement = document.querySelector('[data-table="evm-monthly-metrics"]') as HTMLElement;
    ...
    if (tableElement) {
      ...
      try {
        ...
        link.download = `EVM Monthly Report - ${format(endDate, 'MMM yyyy')}.png`;
        link.href = dataUrl;
        ...
        if (blob) {
          try {
            ...
            toast({
              ...
              description: "EVM monthly report image copied successfully",
              duration: 2000,
            });
          } catch (error) {
              ...
              <TableRow>
                <TableCell colSpan={chains.length + 3} className="text-center py-8">
                  <div className="flex items-center justify-center gap-2">
                    ...
                    <span className="text-muted-foreground">Loading EVM monthly data...</span>
                  </div>
                </TableCell>
              </TableRow>
            ) : error ? (
              ...
              return (
                <TableRow key={data.protocol} className="transition-colors hover:bg-muted/30">
                  ...
                  <TableCell className="text-center">
                    <SixMonthTrendChart data={data.sixMonthTrend} growth={data.monthlyGrowth} />
                  </TableCell>
                </TableRow>
              );
                })}
