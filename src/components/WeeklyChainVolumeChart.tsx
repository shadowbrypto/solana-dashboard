...
export function WeeklyChainVolumeChart({ endDate }: WeeklyChainVolumeChartProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Transform data into stacked format for chart
        const transformedData: ChainVolumeData[] = weekDays.map(day => {
    ...
    fetchData();
  }, [endDate, weekDays, disabledProtocols]);
  ...
  const solanaProtocols = useMemo(() => {
    return protocolData.filter(p => p.chain === 'solana').sort((a, b) => b.volume - a.volume);
  }, [protocolData]);

  const evmProtocols = useMemo(() => {
    return protocolData.filter(p => p.chain === 'evm').sort((a, b) => b.volume - a.volume);
  }, [protocolData]);

  const startDateStr = format(weekDays[0], 'MMM dd');
  const endDateStr = format(weekDays[6], 'MMM dd, yyyy');
  ...
  return (
    <ComponentActions
      componentName="Weekly Chain Volume Distribution"
      filename={`Weekly_Chain_Volume_${format(endDate, 'yyyy-MM-dd')}.png`}
    >
      <div className="rounded-lg border border-border bg-background">
        {/* Header */}
        <div className="flex items-center justify-between px-4 py-3 sm:px-6 sm:py-4 border-b border-border">
          <div>
            ...
            <p className="text-caption text-muted-foreground mt-0.5">
              {startDateStr} - {endDateStr} • 7-day volume by chain
            </p>
          </div>
        </div>
                ...
                {solanaProtocols.length > 0 && (
                  <div className="space-y-1">
                    <div className="flex items-center gap-2 px-3 py-2 bg-purple-500/10 rounded-lg border border-purple-500/20">
                      ...
                      <span className="text-xs text-muted-foreground font-mono ml-auto">{formatVolume(solanaVolume)}</span>
                    </div>
