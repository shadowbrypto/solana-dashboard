...
function updateJSONFile() {
    try {
        ...
        console.log(`[${new Date().toLocaleTimeString()}] Data has been updated in ${outputPath}`);
    } catch (error) {
...
watcher
    .on('change', (path) => {
        console.log(`[${new Date().toLocaleTimeString()}] CSV file changed: ${path}`);
        updateJSONFile();
    })
    .on('add', (path) => {
        console.log(`[${new Date().toLocaleTimeString()}] New CSV file detected: ${path}`);
        updateJSONFile();
    })
    .on('error', error => {
